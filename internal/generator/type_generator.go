...
const STRING_NODE_TYPE = "core.StringNode"
...
func generateInterface(node generator.Node, grammr grammar.Grammar, iface grammar.Interface) {
	fields := []FieldInfo{}
...
func generateImplStruct(node generator.Node, grammr grammar.Grammar, iface grammar.Interface, fields []FieldInfo) {
...
func generateDataStruct(node generator.Node, iface grammar.Interface, fields []FieldInfo) {
	...
	for _, field := range fields {
		...
		if field.HasTokenGetter {
			...
			node.AppendLine("func (i *", iface.Name(), "Data) ", field.Name, "Node() ", STRING_NODE_TYPE, " {")
			node.Indent(func(n generator.Node) {
				n.AppendLine("return i.", field.PName)
			})
			node.AppendLine("}")
