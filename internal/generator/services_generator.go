...
func GenerateServices(grammr grammar.Grammar, packageName string) string {
	...
	node.Indent(func(n generator.Node) {
		...
		n.AppendLine("    service.MustPut(sc, NewDefault", grammr.Name(), "ScopeProvider(sc))")
		n.AppendLine("}")
		n.AppendLine("if !service.Has[", grammr.Name(), "ReferenceLinker](sc) {")
		n.AppendLine("    service.MustPut(sc, NewDefault", grammr.Name(), "ReferenceLinker(sc))")
		n.AppendLine("}")
		n.AppendLine("if !service.Has[", grammr.Name(), "ReferencesConstructor](sc) {")
		n.AppendLine("    service.MustPut(sc, NewDefault", grammr.Name(), "ReferencesConstructor(sc))")
		n.AppendLine("}")
		n.AppendLine("if !service.Has[lexer.Lexer](sc) {")
		n.AppendLine("    service.MustPut[lexer.Lexer](sc, NewLexer())")
		n.AppendLine("}")
		n.AppendLine("if !service.Has[parser.Parser](sc) {")
		n.AppendLine("    service.MustPut[parser.Parser](sc, NewParser(sc))")
		n.AppendLine("}")
		n.AppendLine("if !service.Has[core.SymbolContainers](sc) {")
		n.AppendLine("    service.MustPut[core.SymbolContainers](sc, NewSymbolContainers())")
		n.AppendLine("}")
	})
	node.AppendLine("}")
	node.AppendLine()
