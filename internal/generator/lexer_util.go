...
type TokenType struct {
	...
	// Use [GenerateTokenTypesResult.TokenTypes.ByTokenIndex] to
	// retrieve the token type from its token index.
	// TokenIndex is not the index of the token types in [GenerateTokenTypesResult.TokenTypes.All].
	TokenIndex int
...
type tokenTypeLookup struct {
	// Get all token types that were created.
	// Be ware that the token index is NOT the index to the right token type in this array.
	All []TokenType
...
// The max token index needs to be computed (currently) because the index
// assignment process has a overlapping in standalone keywords and keywords
// that are defined as token rule. Plus their is a builtin token type EOF
// at index 0.
func getMaxTokenIndex(r GenerateTokenTypesResult) int {
	maxId := 0
	for _, tokenType := range r.TokenTypes.All {
...
// (TokenIndex) to a token name.
func (r GenerateTokenTypesResult) TokenTypeNamesByTokenIndex() []string {
	maxTokenIndex := getMaxTokenIndex(r)
	names := make([]string, maxTokenIndex+1)
	for _, tokenType := range r.TokenTypes.All {
				...
				if _, ok := alreadyAdded[tokenIndex]; !ok {
					if source, ok := result.TokenIndex.SourceType[tokenIndex]; ok && source != SourceGroup {
						if source == SourceKeyword {
							current.ModeTokenTypes.Keywords = append(current.ModeTokenTypes.Keywords, tokenIndex)
						} else {
							current.ModeTokenTypes.Tokens = append(current.ModeTokenTypes.Tokens, tokenIndex)
						}
