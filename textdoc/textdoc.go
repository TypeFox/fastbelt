...
// Mapper provides position-to-offset conversion operations for a text document.
...
type Overlay struct {
	// Immutable fields (no lock needed after creation)
	uri        protocol.DocumentURI
	languageID string
...
// This is an immutable field, so no lock is needed.
...
// This is an immutable field, so no lock is needed.
func (o *Overlay) LanguageID() string {
	return o.languageID
}
...
func (o *Overlay) Content() ([]byte, error) {
	...
	result := make([]byte, len(o.content))
	copy(result, o.content)
	return result, nil
}
...
func (o *Overlay) Text(r *protocol.Range) string {
	...
	if r != nil {
		start := o.offsetAtLocked(r.Start)
		end := o.offsetAtLocked(r.End)
		return string(o.content[start:end])
	}
...
func (o *Overlay) PositionAt(offset int) protocol.Position {
	...
	offset = max(min(offset, len(o.content)), 0)
	...
	if len(lineOffsets) == 1 {
		...
		offset = o.ensureBeforeEOLLocked(offset, lineOffsets[0])
	...
	offset = o.ensureBeforeEOLLocked(offset, lineOffsets[line])
	return protocol.Position{
		Line:      uint32(line),
...
func (o *Overlay) OffsetAt(position protocol.Position) int {
	o.mu.RLock()
	defer o.mu.RUnlock()
	return o.offsetAtLocked(position)
}
...
func (o *Overlay) offsetAtLocked(position protocol.Position) int {
	...
	if int(position.Line+1) < len(lineOffsets) {
		...
		nextLineOffset = len(o.content)
	}

	offset := min(lineOffset+int(position.Character), nextLineOffset)
	return o.ensureBeforeEOLLocked(offset, lineOffset)
}
...
func (o *Overlay) LineCount() int {
	o.mu.RLock()
	defer o.mu.RUnlock()
	return len(o.getLineOffsetsLocked())
}
...
func (o *Overlay) ensureBeforeEOLLocked(offset, lineOffset int) int {
	for offset > lineOffset && isEOL(o.content[offset-1]) {
		offset--
	}
	return offset
}
