...
func TestInlayHintResolve_AdvertisedOnlyWhenSupported(t *testing.T) {
	...
	if opts := plainInit.Capabilities.InlayHintProvider; assert.NotNil(t, opts, "expected InlayHintProvider capability to be advertised") {
	...
	assert.NoError(t, err)
	if opts := resolvingInit.Capabilities.InlayHintProvider; assert.NotNil(t, opts, "expected InlayHintProvider capability to be advertised") {
