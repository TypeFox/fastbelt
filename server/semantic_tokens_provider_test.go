...
func TestSemanticTokensProvider_Default(t *testing.T) {
	...
	// The provider itself is always registered: it performs real framework
	// work (document lookup, token iteration, LSP encoding) regardless of
	// whether a contributor is configured.
	provider := service.MustGet[server.SemanticTokensProvider](sc)
	assert.NotNil(t, provider)

	// SemanticTokensContributor has no default implementation: token
	// classification is entirely language-specific, so it is simply absent
	// until a language adopter registers one.
	assert.False(t, service.Has[server.SemanticTokensContributor](sc), "SemanticTokensContributor should not be auto-registered")
}
...
func TestSemanticTokensProvider_CustomContributor(t *testing.T) {
	...
	service.Put[server.SemanticTokensContributor](sc, &grammarSemanticTokensContributor{})
	sc.Seal()
...
func TestSemanticTokensProvider_TokenTypesAndModifiers(t *testing.T) {
	...
	service.Put[server.SemanticTokensContributor](sc, contributor)
	sc.Seal()
