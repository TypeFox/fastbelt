...
func TestCodeActionResolve_AdvertisedOnlyWhenSupported(t *testing.T) {
	...
	if opts := plainInit.Capabilities.CodeActionProvider; assert.NotNil(t, opts, "expected CodeActionProvider capability to be advertised") {
	...
	assert.NoError(t, err)
	if opts := resolvingInit.Capabilities.CodeActionProvider; assert.NotNil(t, opts, "expected CodeActionProvider capability to be advertised") {
