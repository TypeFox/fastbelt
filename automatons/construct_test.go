	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if nfa.GetStateCount() < 2 {
				t.Errorf("Expected at least 2 states, got %d", nfa.GetStateCount())
			}
			...
			transitions := nfa.GetTransitionsBySource()[startState]
			if transitions == nil {
				if tt.expectMatch {
...
func TestConstruct_Repeat(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			nfa, err := Repeat(tt.automaton, tt.min, tt.max)
...
func TestConstruct_Complement(t *testing.T) {
	...
	originalAccepting := charA.GetAcceptingStates()
	complementAccepting := nfa.GetAcceptingStates()
	...
	if nfa.GetStateCount() == charA.GetStateCount() {
...
func TestConstruct_IntersectNFA(t *testing.T) {
	// Create test automata
	rangeAB, err := Consume(NewRuneSet_Range('a', 'b'))
	if err != nil {
		t.Fatalf("Failed to create test automaton: %v", err)
	}

	rangeBC, err := Consume(NewRuneSet_Range('b', 'c'))
	if err != nil {
		t.Fatalf("Failed to create test automaton: %v", err)
	}

	nfa, err := IntersectNFA(rangeAB, rangeBC)
	if err != nil {
...
func TestConstruct_PackageLevelFunctions(t *testing.T) {
	...
	if err1 == nil && err2 == nil {
		if nfa1.GetStateCount() != nfa2.GetStateCount() {
...
func BenchmarkConstruct_Consume(b *testing.B) {
	charset := NewRuneSet_Range('a', 'z')
	b.ResetTimer()

	for i := 0; i < b.N; i++ {
		_, err := Consume(charset)
		if err != nil {
			b.Fatal(err)
		}
	}
...
func BenchmarkConstruct_Alternate(b *testing.B) {
	nfa1, _ := Consume(NewRuneSet_Range('a', 'a'))
	nfa2, _ := Consume(NewRuneSet_Range('b', 'b'))
	nfa3, _ := Consume(NewRuneSet_Range('c', 'c'))
	b.ResetTimer()

	for i := 0; i < b.N; i++ {
		_, err := Alternate(nfa1, nfa2, nfa3)
		if err != nil {
			b.Fatal(err)
		}
	}
...
func BenchmarkConstruct_Concat(b *testing.B) {
	nfa1, _ := Consume(NewRuneSet_Range('a', 'a'))
	nfa2, _ := Consume(NewRuneSet_Range('b', 'b'))
	nfa3, _ := Consume(NewRuneSet_Range('c', 'c'))
	b.ResetTimer()
...
func BenchmarkConstruct_Repeat(b *testing.B) {
	nfa, _ := Consume(NewRuneSet_Range('a', 'a'))
	b.ResetTimer()

	for i := 0; i < b.N; i++ {
		_, err := Repeat(nfa, 0, 3)
		if err != nil {
			b.Fatal(err)
		}
	}
